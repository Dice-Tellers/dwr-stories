...
class TestStories(flask_testing.TestCase):
    ...
    def test_write_story(self):
        ...
        payload = {'text': 'my cat is drinking a gin tonic with my neighbour\'s dog', 'figures': '', 'as_draft': 'a', 'user': 'b'}
        ...
        payload = {'text': 'my cat is drinking a gin tonic with my neighbour\'s dog', 'figures': '#beer#cat#dog#', 'as_draft': False, 'user_id': '1'}
        response = self.client.post('/stories', data=json.dumps(payload), content_type='application/json')
        body = json.loads(str(response.data, 'utf8'))
        ...
        payload = {'text': 'my cat is drinking a beer with my neighbour\'s dog', 'figures': '#beer#cat#dog#', 'as_draft': False, 'user_id': '1'}
        response = self.client.post('/stories', data=json.dumps(payload), content_type='application/json')
        ...
        #     session['id_story'] = 6
        # payload3 = {'text': 'my cat is drinking dog and beer', 'figures': '#beer#cat#dog#', 'as_draft': False, 'user_id': '1'}
        # response = self.client.post('/stories/6', data=json.dumps(payload3), content_type='application/json')
        # self.assertStatus(response, 201)
        # self.assertEqual(response.data.decode('utf8'), 'New story has been published')
        # q = db.session.query(Story).filter(Story.id == count + 1).first()
        # self.assertEqual(q, None)
